...
const Books = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://mern-book-backend-2ccd.onrender.com/api/getall/');
  ...
  const deleteBook = async (bookID) => {
    try {
      const response = await axios.delete(
        `https://mern-book-backend-2ccd.onrender.com/api/delete/${bookID}`
      );
    ...
    try {
      if (!search.trim()) {
        ...
        const response = await axios.get(
          `https://mern-book-backend-2ccd.onrender.com/api/search/${search}`
        );
